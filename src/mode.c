...
int init_modes(int module_number TSRMLS_DC)
{
	...
	/* Register special constants for CTR mode.
	   When LTC was compiled without CTR support, we still define
	   these constants, but they act as non-operations. */
#ifdef CTR_COUNTER_LITTLE_ENDIAN
	REGISTER_LONG_CONSTANT("TOMCRYPT_CTR_LITTLE_ENDIAN", CTR_COUNTER_LITTLE_ENDIAN, CONST_PERSISTENT | CONST_CS);
#else
	REGISTER_LONG_CONSTANT("TOMCRYPT_CTR_LITTLE_ENDIAN", 0, CONST_PERSISTENT | CONST_CS);
#endif
#ifdef CTR_COUNTER_BIG_ENDIAN
	REGISTER_LONG_CONSTANT("TOMCRYPT_CTR_BIG_ENDIAN", CTR_COUNTER_BIG_ENDIAN, CONST_PERSISTENT | CONST_CS);
#else
	REGISTER_LONG_CONSTANT("TOMCRYPT_CTR_BIG_ENDIAN", 0, CONST_PERSISTENT | CONST_CS);
#endif
#ifdef LTC_CTR_RFC3686
	REGISTER_LONG_CONSTANT("TOMCRYPT_CTR_RFC3686", LTC_CTR_RFC3686, CONST_PERSISTENT | CONST_CS);
#else
	REGISTER_LONG_CONSTANT("TOMCRYPT_CTR_RFC3686", 0, CONST_PERSISTENT | CONST_CS);
#endif
