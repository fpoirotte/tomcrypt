...
<?php
    ...
    var_dump(
        ...
        tomcrypt_mac_file($mac, $sub, $key, dirname(__DIR__) . DIRECTORY_SEPARATOR . 'hello.bin', false),
        bin2hex(tomcrypt_mac_string($mac, $sub, $key, $data, true)),
        bin2hex(tomcrypt_mac_file($mac, $sub, $key, dirname(__DIR__) . DIRECTORY_SEPARATOR . 'hello.bin', true))
    );
?>
--EXPECT--
